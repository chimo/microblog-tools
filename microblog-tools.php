...
class twitter_tools {
    ...
    function get_settings() {
        ...
        $this->tweet_format = $this->tweet_prefix.': %s %s'; // TODO: What is this and why are we calling it twice (see line 343)?
    }
    ...
    function populate_settings() {
        ...
        foreach ($this->acct_options as $option) {
            ...
            if (isset($_POST['ubtools_'.$option]) && (!empty($value) || $value == 0)) {
                $this->$option = $value;
            }
        }
    }
...
class aktt_tweet {
     ...
     *
     * @param acct  array containing current WP account information or NULL
     * @return      true if the tweet was generated by this tool, false otherwise
     */
    function tweet_is_post_notification($acct = NULL) { // TODO: Check what happens user changed their prefix after tweeting but before this is called
        global $wpdb;
        if($acct != NULL) { // Check if this tweet is a specific user's post_notification 
            if (substr($this->tw_text, 0, strlen($acct['tweet_prefix'])) == $acct['tweet_prefix']) {
                return true;
            }
        }
        else { // Check if this tweet is a post_notification from any user
            $results = $wpdb->get_col("SELECT tweet_prefix FROM " . $wpdb->prefix);
            
            foreach($results as $prefix) {
                if (substr($this->tw_text, 0, strlen($prefix)) == $prefix) {
                    return true;
                }
            }
        }
...
//        return !empty($this->tw_reply_tweet);
     ...
     *
     * @param acct  array containing current WP account information
     */
    function add($acct) {
        global $wpdb, $aktt;
        $wpdb->query("
        ...
        if ($aktt->create_blog_posts == '1' && !$this->tweet_post_exists() && !$this->tweet_is_post_notification($acct) && (!$aktt->exclude_reply_tweets || !$this->tweet_is_reply())) {
...
function aktt_api_status_show_url($id, $acct = NULL) {
    global $aktt;
    if($acct == NULL) {
        return str_replace('###ID###', $id, $aktt->api_status_show);
    }
    else {
...
function aktt_profile_url($username) { // TODO: Check if we need an $acct param (i.e.: are we calling this function while not being logged in?)
    global $aktt;
    return str_replace('###USERNAME###', $username, $aktt->profile_url);
}
...
function aktt_hashtag_url($hashtag) {
    ...
    return str_replace('###HASHTAG###', $hashtag, $aktt->hashtag_url);
}
...
function aktt_update_tweets() { // TODO: Add argument to only update a single account (for when the "update tweets" button is pressed in the configs)
    ...
    foreach($results as $account) {
        ...
        if ( aktt_oauth_test($account) && ($connection = aktt_oauth_connection($account)) ) {
            $data = $connection->get($account['api_user_timeline']);
        ...
        if(is_array($tweets) && count($tweets)) {
            ...
            foreach ($tweets as $tw_data) {
                if (!$existing_ids || !in_array($tw_data->id, $existing_ids)) {
                    ...
                    if (strtolower($tw_data->user->screen_name) == strtolower($account['twitter_username'])) {
                        $tweet->add($account);
                    }
                }
            }
        }
        aktt_reset_tweet_checking($hash, time(), $account);
        do_action('aktt_update_tweets');
    }
}

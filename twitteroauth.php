...
class TwitterOAuth {
  ...
  public $host = "https://identi.ca/api/"; // TODO: do not hardcode the host
  /* Set timeout default. */
  public $timeout = 30;
  /* Set connect timeout. */
   ...
   */
  function __construct($host_api, $consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) {
	$this->host = $host_api;
   ...
   * @returns a string
   */
  function getAuthorizeURL($url, $token, $sign_in_with_twitter = TRUE) { // Chimo: added $url param
    if (is_array($token)) {
   ...
   *                "screen_name" => "abraham")
   */
  function getAccessToken($url, $oauth_verifier = FALSE) { // Chimo: added $url
    $parameters = array();
